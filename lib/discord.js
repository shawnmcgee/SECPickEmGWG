...
// Slack for the "within the hour" claim. The window's floor is 75 minutes, so
// a healthy sweep lands inside this and keeps the original wording.
 ...
 */
export function buildReminderMessage({ discordUserId, name, games, siteUrl, now = new Date() }) {
  const count = games.length;
  const lines = games.map(
    (g) => `• ${g.away_team} at ${g.home_team} — ${formatKickoffET(g.kickoff_at)}`
  );

  // "within the hour" is only true while the sweep is running on schedule.
  // When a dropped scheduler forces the window wider, the soonest kickoff can
  // be hours out, and claiming otherwise is just wrong -- so drop the timing
  // claim rather than restate it. Every line carries its own kickoff either
  // way, so nothing is lost.
  const soonestMs = Math.min(...games.map((g) => new Date(g.kickoff_at).getTime()));
  const withinTheHour = soonestMs - now.getTime() <= IMMINENT_MINUTES * 60000;
  const verb = count === 1 ? 'locks' : 'lock';
  const tail = withinTheHour ? `${verb} within the hour:` : "you haven't picked yet:";
  const head =
    `<@${discordUserId}> heads up ${name} — ${count} game${count === 1 ? '' : 's'} ` +
    (withinTheHour ? `you haven't picked ${tail}` : tail);
...
export function sampleReminderGames(now = new Date()) {
  ...
  // 55 rather than 65: rounding up to the half hour adds up to 30 more, and a
  // base of 65 could land past the 90-minute IMMINENT_MINUTES threshold, which
  // would flip the test ping to the widened-window wording at random.
  const first = Math.ceil((now.getTime() + 55 * 60000) / HALF_HOUR) * HALF_HOUR;
